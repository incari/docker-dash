...
async function loadContainers() {
  try {
    ...
    renderContainers();
    renderShortcuts(); // Update dashboard states as well
...
function renderShortcuts() {
  ...
  if (grid) {
    grid.innerHTML = shortcuts.map(s => {
      ...
      // Link Logic: Priority to URL, then Port
      const link = s.url ? s.url : `http://${location.hostname}:${s.port}`;
      const subtitle = s.url ? 'Custom URL' : `:${s.port}`;

      // Container Status Logic
      const container = s.container_id ? containers.find(c => c.id === s.container_id) : null;
      let statusHtml = '';
      let controlButtons = '';

      if (container) {
        const isRunning = container.state === 'running';
        statusHtml = `<span class="absolute top-3 left-3 w-3 h-3 rounded-full ${isRunning ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.4)] z-10' : 'bg-red-500 z-10'}"></span>`;

        controlButtons = `
            <div class="flex gap-1 ml-auto mr-2">
                ${isRunning ? `
                   <button onclick="event.stopPropagation(); stopContainer('${container.id}')" title="Stop" class="w-8 h-8 rounded-full bg-gray-700/50 hover:bg-gray-700 flex items-center justify-center text-red-400 hover:text-red-300 transition-colors">
                     <i class="fas fa-stop text-xs"></i>
                   </button>
                   <button onclick="event.stopPropagation(); startContainer('${container.id}')" title="Restart" class="w-8 h-8 rounded-full bg-gray-700/50 hover:bg-gray-700 flex items-center justify-center text-yellow-400 hover:text-yellow-300 transition-colors">
                     <i class="fas fa-redo text-xs"></i>
                   </button>
                ` : `
                   <button onclick="event.stopPropagation(); startContainer('${container.id}')" title="Start" class="w-8 h-8 rounded-full bg-gray-700/50 hover:bg-gray-700 flex items-center justify-center text-green-400 hover:text-green-300 transition-colors">
      ...
      // We use a div with onclick to simulate the link, allowing nested buttons
      return `
      <div class="relative group cursor-pointer" onclick="window.open('${escapeHtml(link)}', '_blank')">
        <div class="block bg-gray-800 rounded-xl p-6 border border-gray-700 hover:border-blue-500 hover:shadow-lg hover:shadow-blue-500/10 transition-all h-full">
          ${statusHtml}

          <div class="flex items-start justify-between gap-4 mb-4 mt-2">
             <div class="flex items-center gap-4 truncate">
               <div class="w-16 h-16 bg-blue-600/10 rounded-xl flex items-center justify-center text-blue-400 shrink-0">
                  ${iconHtml}
               </div>
               <div class="overflow-hidden">
                 <h3 class="font-bold text-lg text-white mb-1 leading-tight truncate px-1">${escapeHtml(s.name)}</h3>
                 <span class="text-xs font-mono text-gray-500 bg-gray-900 px-2 py-1 rounded border border-gray-700 truncate max-w-full inline-block mt-1">${escapeHtml(subtitle)}</span>
               </div>
             </div>

             <!-- Float Actions (Edit/Delete) - Now in flow! -->
             <div class="opacity-0 group-hover:opacity-100 transition-opacity flex gap-2 z-20 shrink-0">
          ...
          <div class="flex items-center justify-between mt-4">
             <p class="text-gray-400 text-sm line-clamp-2 leading-relaxed flex-1 pr-2">${escapeHtml(s.description || 'No description')}</p>
             
             <!-- Controls embedded in bottom row -->
             ${controlButtons}
          </div>
        </div>
      </div>
...
function renderContainers() {
  ...
  grid.innerHTML = containers.map(c => {
    ...
    const mainPort = ports[0] || 0;

    // Check if added
    const isAdded = shortcuts.some(s => s.container_id === c.id);

    return `
      <div class="bg-gray-800 rounded-xl p-5 border border-gray-700 hover:border-blue-500/50 hover:bg-gray-800/80 transition-all group overflow-hidden flex flex-col h-full">
        
        <div class="flex items-start justify-between gap-3 mb-3">
          <div class="flex items-center gap-3 truncate">
            <span class="w-3 h-3 rounded-full ${isRunning ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.4)]' : 'bg-gray-500'} shrink-0"></span>
            <h3 class="font-bold text-gray-200 truncate">${escapeHtml(c.name)}</h3> 
          </div>

          <!-- Action Badges (Now in flow) -->
          <div class="opacity-0 group-hover:opacity-100 transition-opacity flex gap-2 shrink-0">
              ${!isAdded ? `
                <button onclick="event.stopPropagation(); instantAdd('${escapeHtml(c.id)}', '${escapeHtml(c.name)}', ${mainPort})" 
                   class="bg-yellow-500 hover:bg-yellow-400 text-gray-900 text-xs font-bold px-2 py-1.5 rounded-lg shadow-lg flex items-center justify-center min-w-[32px]"
                   title="Add to Favorites (One Click)">
                   <i class="fas fa-star"></i> 
                </button>
              ` : `
                <span class="bg-gray-700 text-green-400 text-xs font-bold px-2 py-1.5 rounded-lg border border-gray-600 flex items-center justify-center min-w-[32px]" title="Added to shortcuts">
                   <i class="fas fa-check"></i>
                </span>
              `}
              
              <button onclick="quickAdd('${escapeHtml(c.name)}', ${mainPort}, '${escapeHtml(c.id)}')" 
                 class="bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold px-3 py-1.5 rounded-lg shadow-lg flex items-center gap-1"
                 title="Customize & Add">
        ...
        <div class="mt-auto pt-4 border-t border-gray-700/50 flex items-center justify-between">
           <div class="flex flex-wrap gap-1">
            ...
            `).join('') || '<span class="text-gray-600 text-xs italic">No ports</span>'}
          </div>
          
          <!-- Control Buttons -->
          <div class="flex gap-2">
            ${isRunning ? `
               <button onclick="stopContainer('${c.id}')" title="Stop" class="text-red-400 hover:text-red-300 p-1"><i class="fas fa-stop"></i></button>
            ` : `
               <button onclick="startContainer('${c.id}')" title="Start" class="text-green-400 hover:text-green-300 p-1"><i class="fas fa-play"></i></button>
            `}
          </div>
        </div>
      </div>
...
async function instantAdd(containerId, name, port) {
  const formData = new FormData();
  formData.append('name', name);
  formData.append('port', port);
  formData.append('container_id', containerId);
  formData.append('icon', 'cube'); // default

  // Default description
  formData.append('description', 'Quick added container');

  try {
    const res = await fetch(`${API_BASE}/api/shortcuts`, { method: 'POST', body: formData });
    if (!res.ok) throw new Error('Failed to save');

    // Refresh data to update buttons
    await loadShortcuts();
    renderContainers(); // Re-render to show "Added" badge
  } catch (err) {
    console.error(err);
...
async function startContainer(id) {
  try {
    const res = await fetch(`${API_BASE}/api/containers/${id}/start`, { method: 'POST' });
...
async function stopContainer(id) {
  ...
  try {
    const res = await fetch(`${API_BASE}/api/containers/${id}/stop`, { method: 'POST' });
    if (res.ok) loadContainers();
  } catch (e) { console.error(e); }
}
...
function openModal(data = null) {
  ...
  // Handle Port vs URL
  if (data?.url) {
    switchConnectionType('url');
    document.getElementById('shortcut-url').value = data.url;
    document.getElementById('shortcut-port').value = '';
  } else {
    switchConnectionType('port');
...
// Connection Type Toggler
function switchConnectionType(type) {
  document.getElementById('connection-type').value = type;

  // Toggle active state of buttons
  const btnPort = document.getElementById('type-port');
  const btnUrl = document.getElementById('type-url');

  if (type === 'port') {
    btnPort.className = 'flex-1 py-1.5 rounded text-sm font-medium transition-colors bg-gray-700 text-white shadow-sm';
    btnUrl.className = 'flex-1 py-1.5 rounded text-sm font-medium text-gray-400 hover:text-white transition-colors';

    document.getElementById('input-port-wrapper').classList.remove('hidden');
    document.getElementById('input-url-wrapper').classList.add('hidden');
  } else {
    btnUrl.className = 'flex-1 py-1.5 rounded text-sm font-medium transition-colors bg-gray-700 text-white shadow-sm';
    btnPort.className = 'flex-1 py-1.5 rounded text-sm font-medium text-gray-400 hover:text-white transition-colors';

    document.getElementById('input-port-wrapper').classList.add('hidden');
    document.getElementById('input-url-wrapper').classList.remove('hidden');
  }
}
...
async function saveShortcut(e) {
  ...
  if (connType === 'port') {
    ...
    formData.append('url', document.getElementById('shortcut-url').value);
  }
  ...
  try {
    ...
    // Logic: If adding (first time list), stay on 'add' view. If manually doing one from dashboard, go back to dashboard.
    // 'view-add' is hidden if we are on dashboard.
    const addView = document.getElementById('view-add');
    if (addView.classList.contains('hidden')) {
      switchView('dashboard');
    }
    // Else stay on ADD view to allow multiple adds
