...
interface ShortcutActions {
  ...
  handleToggleFavorite: (id: number, currentStatus: boolean | number) => Promise<void>;
  handleSaveChanges: (changes: Array<{ type: string; shortcutId: number; sectionId: number | null; position: number }>) => Promise<void>;
}
...
export function useShortcutActions(
  ...
  const handleQuickAdd = useCallback(async (container: DockerContainer) => {
    ...
    try {
      ...
      onError("Error Adding Shortcut", err.response?.data?.error || "Failed to add shortcut");
    }
  }, [onRefresh, onError]);

  const handleToggleFavorite = useCallback(async (id: number, currentStatus: boolean | number) => {
    try {
  ...
  const handleSaveChanges = useCallback(async (
    changes: Array<{ type: string; shortcutId: number; sectionId: number | null; position: number }>
  ) => {
    ...
    try {
      await Promise.all(
        changes.map(change =>
          shortcutsApi.updateSection(change.shortcutId, change.sectionId, change.position)
        )
      );
    } catch (err) {
